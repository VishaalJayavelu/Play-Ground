...
const Launch = props => {
  const selectorBody = useMemo(() => {
    return props.planets?.map(planets => 
      <option value={planets.kepler_name} key={planets.kepler_name}>{planets.kepler_name}</option>
    );
  }, [props.planets]);
  ...
  return <Appear id="launch" animate show={props.entered}>
    ...
    <form onSubmit={props.submitLaunch} style={{display: "inline-grid", gridTemplateColumns: "auto auto", gridGap: "10px 20px"}}>
      ...
      <input type="text" id="rocket-name" name="rocket-name"/>
